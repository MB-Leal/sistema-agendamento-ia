<x-app-layout>
    ...
    <div class="w-full mx-auto sm:px-6 lg:px-8 my-4">
        ...
        <div class="flex h-[calc(100vh-320px)] min-h-[450px] bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div class="w-1/3 min-w-[280px] max-w-[400px] border-r border-gray-200 flex flex-col h-full bg-white">
                ...
                <div class="p-2 bg-white border-b border-gray-100 shrink-0">
                    <div class="relative w-full">
                        <input type="text" id="searchChat" class="w-full bg-gray-100 rounded-lg border-none text-xs py-2 pl-9 pr-4 placeholder-gray-500 focus:bg-white focus:ring-2 focus:ring-emerald-500 focus:outline-none" placeholder="Pesquisar conversa">
                ...
                <div class="flex-1 overflow-y-auto divide-y divide-gray-100 style-scrollbar bg-white">
                    @forelse($contacts as $contact)
                        ...
                        <a href="?connection_id={{ $selectedConnectionId }}&chat={{ $contact->remote_jid }}" 
                           class="flex items-center h-[72px] px-3 hover:bg-gray-50 transition duration-150 contact-item {{ $activeChat == $contact->remote_jid ? 'bg-gray-100' : '' }}"
                            ...
                            <div class="flex-1 min-w-0 ml-3 h-full flex flex-col justify-center border-b border-gray-100 py-1">
                                ...
                                <div class="flex justify-between items-center mt-0.5">
                                    ...
                                    <div class="flex gap-1 items-center shrink-0 ml-2">
                                        ...
                                        @if($hasUnread)
                                            <span class="inline-flex items-center justify-center h-4 min-w-4 px-1 rounded-full text-[9px] font-bold bg-emerald-500 text-white">{{ $contact->unread_count }}</span>
                                        @endif
                                    </div>
                                </div>
                            </div>
            ...
            <div class="flex-1 flex flex-col h-full bg-[#efeae2] relative">
                @if($activeChat)
                    <div class="h-14 px-4 bg-gray-50 border-b border-gray-200 flex items-center justify-between shrink-0 z-10">
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <small class="text-emerald-600 font-semibold text-xs">Sincronizado</small>
                            </div>
                        </div>
                    </div>

                    <div class="flex-1 p-4 overflow-y-auto flex flex-col gap-2 bg-whatsapp-pattern style-scrollbar" id="messagesBox">
                    ...
                    <div class="h-14 px-4 bg-gray-50 border-t border-gray-200 flex items-center shrink-0 z-10">
                        <form method="POST" action="{{ route('admin.whatsapp.send') }}" class="flex w-full gap-3 items-center">
                            ...
                            <input type="text" name="message" class="flex-1 rounded-lg border-gray-300 shadow-xs text-sm py-2 px-4 bg-white placeholder-gray-400 focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500" placeholder="Digite uma mensagem" required autocomplete="off">
