...
class OpenAIService
{
    ...
    public function getAIResponse(string $phoneContact, string $customerMessage): string
    {
        ...
        // 1. SEGMENTAÇÃO E AGREGADO DE DADOS DO BANCO DE DADOS REAL
        $nomeCliente = null;
        ...
        try {
            // 🛡️ CORREÇÃO CIRÚRGICA: Busca usando a coluna real 'telefone' identificada no dump SQL
            $user = User::where('telefone', $phoneContact)
                        ->orWhere('telefone', 'like', '%' . substr($phoneContact, 4) . '%')
                        ->first();
            ...
            if ($user) {
                ...
                // Busca as reservas confirmadas deste cliente para hoje ou datas futuras
                ...
                if ($reservasAtivas->count() > 0) {
                    $contextoReservas = "RESERVAS ATIVAS ENCONTRADAS NO SEU SISTEMA:\n";
                    foreach ($reservasAtivas as $reserva) {
                        // Combina data e hora para calcular a janela de cancelamento de 24h
                        $dataCarb = Carbon::parse($reserva->data_reserva . ' ' . $reserva->hora_inicio);
                        $podeCancelar = Carbon::now()->diffInHours($dataCarb, false) >= 24;
                        
                        $contextoReservas .= "- Reserva ID: {$reserva->id} | Data: " . Carbon::parse($reserva->data_reserva)->format('d/m/Y') . " às " . substr($reserva->hora_inicio, 0, 5) . "h | ";
                        $contextoReservas .= $podeCancelar 
                            ? "STATUS CANCELAMENTO: Permitido (Faltam mais de 24 horas. O cliente pode cancelar ou trocar de horário se pedir).\n" 
                            : "STATUS CANCELAMENTO: Bloqueado (Faltam menos de 24 horas. Se ele pedir para cancelar/alterar, explique de forma educada que a política da Arena impossibilita o reembolso do sinal por conta da proximidade).\n";
                    }
                }
            }
        ...
        $systemPrompt = "Você é a assistente virtual inteligente e atendente oficial da Arena Elizeu.\n" .
                        ...
                        "📌 CONTEXTO REAL DO CLIENTE ATUAL (BUSCADO NO BANCO DE DADOS EM TEMPO REAL):\n" .
                        "{$contextoCliente}\n" .
                        "{$contextoReservas}\n\n" .
                        ...
                        "- Quando o usuário confirmar o Nome, o Dia, o Horário e concordar em pagar os R$ 50,00 de sinal por PIX, você deve aceitar e incluir obrigatoriamente a tag exata [GERAR_PIX:50.00] colada ao final do seu texto de resposta.\n\n" .
                        "💬 REGRAS DE CONVERSAÇÃO:\n" .
                        "- Respostas curtas (máximo 3 frases), ideais para leitura rápida no WhatsApp.\n" .
                        "Hoje é dia " . date('d/m/Y') . " (Horário atual: " . date('H:i') . ").";

        // 3. RECUPERAÇÃO DA MEMÓRIA TEXTUAL RECENTE
        $historyMessages = [];
        try {
